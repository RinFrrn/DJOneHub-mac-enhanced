...
| 操作 | 作用 | 是否修改模块状态 |
|---|---|---|
| `STATUS` | 读取 WMS 协议、传输注册状态和实机 IDL 版本 | 否 |
...
|---|---|
| `STATUS` | Get Message Protocol `0x0030`；Get Transport Registration `0x004a` |
| `LIST` | List Messages `0x0031` |
| `READ` | Raw Read `0x0022` |
| `SEND_RAW` | Raw Send `0x0020` |
| `DELETE` | Delete `0x0024` |

列表兼容 tag TLV `0x11`/`0x02` 以及带或不带 GW mode 的变体；删除兼容
index/mode TLV `0x10`/`0x12` 与 `0x02`/`0x04` 两种布局。读取兼容带 tag 和不带 tag
的 WMS raw-message 返回值。所有 QMI 服务结果 TLV 都必须被解析并确认为成功，不能只以
CCI transport 返回值判断成功。

模块 vendor `libqmiservices.so.1` 不提供编译期头文件，因此运行时只解析固定符号
`wms_get_service_object_internal_v01`。网关固定 major version 1，并在初始化时发现模块
实际支持的 minor/tool 组合；发现值会通过 `STATUS` 返回，后续可在实机验收后固定下来。
...
2. 以 `--read-only --once` 启动，使用现有 pairing key 完成一次 `STATUS`。
3. 记录实际 WMS IDL major/minor/tool、消息协议和 transport registration。
4. 以 `--read-only` 启动，分别对 NV (`1`) 和 SIM (`0`) 做 `LIST`；对一个已知索引
   做 `READ`，在 Mac 侧解码 PDU 并与模块现有短信数据核对。
5. 使用专门测试号码发送一条单段短信；先保留原始 PDU 和 QMI message ID，再由对端
   确认内容。此步骤才启用完整权限。
6. 删除仅针对本次测试短信的精确 storage/index，并在删除前后各做一次 `LIST`。
7. 同时拨打一通电话，确认 voice control `45750` 和 PCM 帧计数不受短信 daemon
   启停或 WMS 请求影响。

在完成第 2 至第 4 步之前，iOS 信息页不应显示“短信可用”；在完成一次真实发送与接收
闭环之前，也不应对用户暴露发送按钮。
