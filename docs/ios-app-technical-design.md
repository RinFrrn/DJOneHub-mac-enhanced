...
- “信息”通过独立的认证 SMS 端口读取 QDC507 的 SIM/NV 消息列表和原始 3GPP PDU，
  并在本机解码 GSM 7-bit、UCS-2 与 8-bit 文本；它不尝试读取 iOS 系统短信数据库。
  前台每 5 秒自动刷新，新短信在列表中显示未读样式并同步到 Tab badge，进入详情后将
  本机阅读状态持久标记为已读。当前模块会话与页面均保持只读，在真实发送闭环验收前
  不显示发送按钮。
...
- 全屏来电/通话界面、静音和显式本地录音。
- 诊断信息从主流程下沉到设置。
- 已加入独立 QDC507 SMS 认证协议及 iOS 只读收件页；来电号码和短信发送仍待验收。
